...
test('a refused Show clears the earlier table and count', async ({ page, context, request }) => {
  await fresh(context, request)
  await twoDays(request)
  await staffSignsIn(page)
  await openTab(page, 'Contact list')
  ...
  await expect(page.locator('#contacts-count')).toHaveText('4 visits')

  await page.locator('#contacts-from').fill('2026-09-14')
  await page.locator('#contacts-to').fill('2026-09-13')
  const refused = page.waitForResponse((r) => new URL(r.url()).pathname === '/api/staff/contacts')
  await tap(page, page.locator('#contacts-show'), 'Show again')
  const r = await refused
  expect(r.status()).toBe(400)
  const body = await r.json()
  expect(body).toMatchObject({ field: 'to', error: 'The end date is before the start date.' })
  await expect(page.locator('#contacts-form [data-error-for="to"]')).toHaveText(body.error)
  await expect(rows, 'no rows stay after a refused Show').toHaveCount(0)
  await expect(page.locator('#contacts-count'), 'no count stays after a refused Show').toHaveText('')
  assertNoThirdParty(context)
})
